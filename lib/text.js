...
function renderChar(char, font="default"){
    ...
    if (Math.random() < 0.01 && font == "segment" && char != "_"){if (Math.random() < 0.1){char = String(Math.round(Math.random()*9))} else {char = "_"}}
    if (letterIndex("numb", char)[0]){
        return `<div style="width: 8px; height: 16px; background-image: url(img/letter/${font}/num.png); background-position: ${(letterIndex("numb", char)[1]+1)*8}px 0px"></div>`
    }
    if (letterIndex("char", char)[0]){
        return `<div style="width: 8px; height: 16px; background-image: url(img/letter/${font}/cha.png); background-position: ${(letterIndex("char", char)[1]+1)*8}px 0px"></div>`
    }
    return `<div style="width: 8px; height: 16px; background-image: url(img/letter/${font}/${char[0]}.png);"></div>`
}
 ...
 * @returns {string} HTML UFO 50 Text, apply to an element's `.innerHTML`
 */
function renderString(string, font="default"){
 ...
 */
function renderStrings(strings, font="default"){
    let newString = ``

    for (let x = 0; x < strings.length; x++){
        newString += renderString(strings[x], font=font)
    }
...
function letterIndex(type, char){
    let index = {
        "numb": "0123456789",
        "char": "abcdefghijklmnopqrstuvwxyz"
    }

    for (let x = 0; x < index[type].length; x++){
        if (char == index[type][x]){
            return [true, x]
        }
    }
